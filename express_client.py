...
class ExpressClient:
    ...
    async def create_category(self, name: str):
        return await self._req("POST", "/api/categories", json={"name": name})
    ...
    async def edit_category(self, category_id: str, name: str, is_selected: bool):
        return await self._req(
            "PUT", f"/api/categories/{category_id}",
            json={"name": name, "is_selected": is_selected},
        )
